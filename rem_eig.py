...
routeDict = {}
to_rem_eig = list(Ei_cc_Dict.keys())
#rem_edges = ord_bet
...
if nx.has_path(J1, s, t):
      ...
      nx.write_graphml(J1,"SP_Eig_Bef_removal.graphml")
else:
...
if nx.has_path(J2, s, t):      
      for path in effected_shortest_paths(J2, s, t, 1):
            ...
            for i in range(len(path)-1):
                  n = path[i]
                  J2.node[n]['color']='red'
                  J1.add_path(n,color= 'red' )
                  total_length += 1
            print("Shortest path After removal")
            print('{}: {}'.format(path, total_length))
      nx.write_graphml(J2,"SP_Eig_Aft_removal.graphml")
else:
      print("No Path between these nodes, Kindly run again!!")
